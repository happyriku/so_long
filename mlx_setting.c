...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/12 19:36:47 by rishibas          #+#    #+#             */
/*   Updated: 2024/07/14 19:35:20 by rishibas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	info->image_info.player = mlx_xpm_file_to_image(info->mlx, PLAYER_IMAGE,
			&info->image_info.size, &info->image_info.size);
	if (info->image_info.player == NULL)
		print_error(info, "failed to get player image");
	info->image_info.exit = mlx_xpm_file_to_image(info->mlx, EXIT_IMAGE,
			&info->image_info.size, &info->image_info.size);
	if (info->image_info.exit == NULL)
		print_error(info, "failed tp get exit image");
	info->image_info.item = mlx_xpm_file_to_image(info->mlx, ITEM_IMAGE,
			&info->image_info.size, &info->image_info.size);
	if (info->image_info.item == NULL)
		print_error(info, "failed to get item image");
	info->image_info.tile = mlx_xpm_file_to_image(info->mlx, TILE_IMAGE,
			&info->image_info.size, &info->image_info.size);
	if (info->image_info.tile == NULL)
		print_error(info, "failed to get tile image");
	info->image_info.wall = mlx_xpm_file_to_image(info->mlx, WALL_IMAGE,
			&info->image_info.size, &info->image_info.size);
	if (info->image_info.wall == NULL)
		print_error(info, "failed to get wall image");
	info->image_info.rasengan = mlx_xpm_file_to_image(info->mlx, RASENGAN_IMAGE,
			&info->image_info.size, &info->image_info.size);
